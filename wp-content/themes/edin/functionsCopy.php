...
function health_information() {
	...
	$custom = get_post_custom( $_GET['post'], 'SMTH_METANAME_VALUE' , true );
	//$custom = get_post_custom($post -> ID);
	$health_information = $custom['health_information'][0];
    wp_editor( htmlspecialchars_decode($health_information), 'mettaabox_ID_stylee', $settings = array('textarea_name'=>'health_information') );
}
 
function admin_init() {
	add_meta_box('health_information-meta', 'Health Information', 'health_information', 'states');
}
...
function save_details() {
	...
	update_post_meta($post -> ID, 'health_information', $_POST['health_information']);
	/*
    if (!empty($_POST['health_information'])) {
        $datta = htmlspecialchars($_POST['health_information']);
        update_post_meta($post -> ID, 'SMTH_METANAME_VALUE', $datta );
    }*/
}
...
/*
add_action('init', 'answers_register');
 
function answers_register() {
 
	$labels = array(
		'name' => _x('Answers', 'post type general name'),
		'singular_name' => _x('Answers Content', 'post type singular name'),
		'add_new' => _x('Add New', 'answers content'),
		'add_new_item' => __('Add New Answers Content'),
		'edit_item' => __('Edit Answers Content'),
		'new_item' => __('New Answers Content'),
		'view_item' => __('View Answers Content'),
		'search_items' => __('Search Answers Content'),
		'not_found' =>  __('Nothing found'),
		'not_found_in_trash' => __('Nothing found in Trash'),
		'parent_item_colon' => ''
	);
	...
	$args = array(
		...
		'show_ui' => true,
		'query_var' => true,
		'menu_icon' => get_stylesheet_directory_uri() . '/images/custom-content.png',
		'rewrite' => true,
		'capability_type' => 'post',
		'hierarchical' => false,
		'menu_position' => null,
...
*/
//register_taxonomy('States', array('healthcare'), array('hierarchical' => true, 'label' => 'States', 'singular_label' => 'State', 'rewrite' => true));
//register_taxonomy('Answers', array('healthcare'), array('hierarchical' => true, 'label' => 'Answers', 'singular_label' => 'Answer', 'rewrite' => true));
...
function year_completed(){
	global $post;
	$custom = get_post_custom ($post -> ID);
	$year_completed = $custom['accordion'][0];
	?>
	<div>Year:</div>
...
/**
 * Enable HTML tags for widgets titles
 *
 * @return string
 */
/*
